	...
	render() {
		...
		return (
			<div id="PublicationSingleViewerContainer">
				...
				<div className="cardFather">
					<button className="btn btn-outline-success" onClick={this.refresh}>
						Refresh
					</button>
				</div>
				<div className="cardFather">
					<DynamicTablePublications
						className="c5pxmarginstop"
						elements={this.state.publications}
						refresh={this.refresh.bind()}
						deleteRecord={this.deleteRecord.bind()}
					></DynamicTablePublications>
				</div>
			</div>
		);
